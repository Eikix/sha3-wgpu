...
function BasicDemo() {
  ...
  const handleBatchHash = async () => {
    ...
    try {
      ...
      setOutput((prev) => [
        ...prev,
        `Hashing ${messages.length} messages in one GPU batch...`,
        "",
      ]);
      ...
      setOutput((prev) => [
        ...prev,
        "",
        `Successfully hashed ${hashes.length} messages on GPU!`,
      ]);
    } catch (error) {
      setOutput((prev) => [...prev, `Error: ${error}`, ""]);
    } finally {
  ...
  if (hasherLoading) {
    return (
      <div>
        <h2>Basic Usage</h2>
        <p>
          Demonstrate single hash and batch hashing using GPU-accelerated SHA-3.
        </p>
        <div className="output">
          <div className="output-line info">
            Initializing GPU hasher... Please wait.
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div>
      ...
      {!hasher && !hasherLoading && !hasherError && (
        <div className="output">
          <div className="output-line error">
            Hasher not initialized. Please check the browser console for errors.
          </div>
        </div>
      )}
      ...
      <div className="controls">
        ...
        <button
          ...
          disabled={loading || !hasher || !inputText}
        >
          Hash Single
      ...
      <div className="controls">
        ...
        <button
          ...
          disabled={loading || !hasher || !batchInputs.trim()}
        >
          Hash Batch
          {loading && <span className="loading"></span>}
        </button>
